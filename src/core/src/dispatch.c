...
#if RTR_ENABLE_AVX2
                if (ebx & (1u << 5)) mask |= 1u << RTR_SIMD_AVX2;
#endif
...
static const rtr_xxh3_kernel_t *rtr_kernel_for(rtr_simd_level_t level) {
    switch (level) {
#if RTR_ENABLE_NEON
        case RTR_SIMD_NEON: return &rtr_kernel_neon;
#endif
#if RTR_ENABLE_SSE2
        case RTR_SIMD_SSE2: return &rtr_kernel_sse2;
#endif
#if RTR_ENABLE_AVX2
        case RTR_SIMD_AVX2: return &rtr_kernel_avx2;
#endif
#if RTR_ENABLE_AVX512
        case RTR_SIMD_AVX512: return &rtr_kernel_avx512;
#endif
        default: return &rtr_kernel_scalar;
    }
}
...
static rtr_simd_level_t rtr_best_of(uint32_t mask) {
    for (int lvl = RTR_SIMD_AVX512; lvl > RTR_SIMD_SCALAR; lvl--)
        if (mask & (1u << lvl)) return (rtr_simd_level_t)lvl;
...
static rtr_simd_level_t rtr_active(void) {
    ...
    if (forced >= 0) return (rtr_simd_level_t)forced;
    (void)rtr_levels();
...
rtr_simd_level_t rtr_hash_init(void) { return rtr_active(); }
...
rtr_simd_level_t rtr_hash_active_level(void) { return rtr_active(); }

uint32_t rtr_hash_available_levels(void) { return rtr_levels(); }

int rtr_hash_force_level(rtr_simd_level_t level) {
    uint32_t const mask = rtr_levels();
    int const lvl = (int)level;

    if (lvl < (int)RTR_SIMD_SCALAR || lvl > (int)RTR_SIMD_AVX512) return -1;
    if (!(mask & (1u << (unsigned)lvl))) return -1;
    RTR_STORE(g_forced, lvl);
    return 0;
}

void rtr_hash_reset_level(void) { RTR_STORE(g_forced, -1); }

const char *rtr_hash_level_str(rtr_simd_level_t level) {
    switch (level) {
        case RTR_SIMD_SCALAR: return "scalar";
        case RTR_SIMD_NEON: return "neon";
        case RTR_SIMD_SSE2: return "sse2";
        case RTR_SIMD_AVX2: return "avx2";
        case RTR_SIMD_AVX512: return "avx512";
...
rtr_hash_state_t *rtr_hash_create(void) {
    ...
    if (st != NULL) rtr_xxh3_state_reset(st, 0);
    return st;
}

void rtr_hash_destroy(rtr_hash_state_t *state) { RTR_FREE(state); }
...
int rtr_hash_update(rtr_hash_state_t *state, const void *data, size_t len) {
    if (state == NULL) return -1;
    if (len == 0) return 0; /* including data == NULL */
    if (data == NULL) return -1;
    rtr_xxh3_state_update(state, (const uint8_t *)data, len, rtr_active_kernel());
    return 0;
}
