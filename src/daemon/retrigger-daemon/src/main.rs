...
use retrigger_system::{SystemWatcher, FileEventProcessor};
...
async fn generate_config(args: ConfigArgs) -> Result<()> {
    if args.output.exists() && !args.force {
        anyhow::bail!("Configuration file already exists: {}", args.output.display());
    }
...
async fn run_benchmark(args: BenchmarkArgs) -> Result<()> {
    ...
    println!("Events/sec: {:.2}", events_received as f64 / event_time.as_secs_f64());
    println!("Avg latency per event: {:?}", event_time / events_received as u32);
    
    // Get cache statistics
    let (cache_entries, cache_capacity) = processor.cache_stats();
    println!("Hash cache utilization: {}/{} ({:.1}%)", 
...
async fn init_metrics(config: &DaemonConfig) -> Result<()> {
    ...
    info!("Metrics endpoint started on port {}", config.server.metrics_port);
    Ok(())
}
