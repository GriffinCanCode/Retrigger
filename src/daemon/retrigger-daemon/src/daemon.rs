...
use retrigger_system::{SystemWatcher, FileEventProcessor, EnhancedFileEvent};
use tokio::sync::broadcast;
use tracing::{debug, error, info, warn};

use crate::config::{ConfigManager, DaemonConfig, CompiledPatterns};
use crate::grpc::GrpcServer;
use crate::metrics::MetricsCollector;
use crate::ipc::{ZeroCopyRing, ZeroCopyConfig};
...
impl Daemon {
    ...
    pub async fn new(config_manager: ConfigManager) -> Result<Self> {
        ...
        let system_watcher = Arc::new(SystemWatcher::new()
            .with_context(|| "Failed to create system watcher")?);
        ...
        let ipc_ring = match ZeroCopyRing::create_producer(ipc_config) {
            ...
            Err(e) => {
                warn!("Failed to create IPC ring buffer: {}, continuing without IPC", e);
                None
            }
        };
        ...
        for watch_path in &config.watcher.watch_paths {
            if watch_path.enabled {
                self.system_watcher
                    ...
                    .await
                    .with_context(|| format!("Failed to watch directory: {}", watch_path.path.display()))?;
            }
        }
    ...
    async fn process_event_batch(
        ...
        for event in events {
            match processor.process_event(event.clone()).await {
                ...
                Err(e) => {
                    warn!("Failed to process event for {}: {}", event.path.display(), e);
    ...
    async fn start_metrics_collector(&self) -> Result<()> {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                event_processor.cleanup_cache(Duration::from_secs(3600)).await;
            }
        });
