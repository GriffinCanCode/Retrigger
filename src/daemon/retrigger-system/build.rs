...
fn main() {
    ...
    let zig_dir = Path::new(&manifest_dir).parent().unwrap().parent().unwrap().join("system/zig");
    ...
    match output {
        Ok(output) => {
            if !output.status.success() {
                println!("cargo:warning=Zig build failed: {}", String::from_utf8_lossy(&output.stderr));
                println!("cargo:warning=Falling back to stub implementation");
                return;
            }
        }
        Err(e) => {
            println!("cargo:warning=Zig not found ({}), using stub implementation", e);
            return;
        }
    }
