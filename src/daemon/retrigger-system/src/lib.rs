...
use dashmap::DashMap;
use retrigger_core::{HashEngine, HashResult, FastHash};
...
mod ffi {
    ...
    extern "C" {
        ...
        pub fn fw_watcher_poll_event(
            watcher: *mut FileWatcher,
            out_event: *mut FileEvent,
        ) -> bool;
    }
}
...
impl SystemWatcher {
    ...
    pub fn new() -> Result<Self> {
        ...
        info!("Created system watcher with SIMD level: {:?}", hash_engine.simd_level());
    ...
    pub async fn watch_directory<P: AsRef<Path>>(&self, path: P, recursive: bool) -> Result<()> {
        ...
        let result = unsafe {
            ffi::fw_watcher_watch_directory(self.watcher, c_path.as_ptr(), recursive)
        };
        ...
        info!("Watching directory: {} (recursive: {})", path.display(), recursive);
        Ok(())
    }
    ...
    async fn event_polling_task(
        ...
        loop {
            ...
            let has_event = unsafe {
                ffi::fw_watcher_poll_event(watcher, &mut ffi_event)
            };
...
impl FileEventProcessor {
    ...
    pub async fn process_event(&self, event: SystemEvent) -> Result<EnhancedFileEvent> {
        ...
        let hash = if !event.is_directory && 
                     matches!(event.event_type, SystemEventType::Created | SystemEventType::Modified) {
    ...
    fn invalidate_directory(&self, dir: &Path) {
        ...
        self.directory_cache.retain(|path, _| {
            !path.to_string_lossy().starts_with(dir_str.as_ref())
        });
    }
