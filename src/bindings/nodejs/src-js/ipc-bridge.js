...
class RustIPCBridge extends EventEmitter {
  ...
  async connect() {
    try {
      ...
      if (bytesRead !== stats.size) {
        throw new Error(`Failed to read complete memory map: ${bytesRead}/${stats.size}`);
      }
  ...
  async waitForFile(timeoutMs = 5000) {
    ...
    throw new Error(`Rust daemon IPC file not found after ${timeoutMs}ms: ${this.mmapPath}`);
  }
  ...
  validateHeader() {
    ...
    if (magic !== MAGIC_NUMBER) {
      throw new Error(`Invalid magic number: 0x${magic.toString(16)} (expected 0x${MAGIC_NUMBER.toString(16)})`);
    }
  ...
  registerConsumer() {
    ...
    this.buffer.writeUInt32LE(this.consumerPid, MEMORY_LAYOUT.CONSUMER_PID_OFFSET);
  }
  ...
  processEvents() {
    ...
    try {
      ...
      // Check for shutdown
      const shutdownFlag = this.buffer.readUInt32LE(MEMORY_LAYOUT.SHUTDOWN_FLAG_OFFSET);
      if (shutdownFlag !== 0) {
        this.emit('shutdown');
        return;
      }
  ...
  deserializeEvent(offset) {
    try {
      // Read event fields
      const timestamp = this.buffer.readBigUInt64LE(offset + MEMORY_LAYOUT.EVENT_TIMESTAMP_OFFSET);
      const eventType = this.buffer.readUInt32LE(offset + MEMORY_LAYOUT.EVENT_TYPE_OFFSET);
      const pathLen = this.buffer.readUInt32LE(offset + MEMORY_LAYOUT.EVENT_PATH_LEN_OFFSET);
      const size = this.buffer.readBigUInt64LE(offset + MEMORY_LAYOUT.EVENT_SIZE_OFFSET);
      const isDirectory = this.buffer.readUInt32LE(offset + MEMORY_LAYOUT.EVENT_IS_DIR_OFFSET) !== 0;
      const hashPresent = this.buffer.readUInt32LE(offset + MEMORY_LAYOUT.EVENT_HASH_PRESENT_OFFSET) !== 0;
      const hashValue = this.buffer.readBigUInt64LE(offset + MEMORY_LAYOUT.EVENT_HASH_VALUE_OFFSET);
  ...
  getStats() {
    ...
    try {
      ...
      const capacity = this.buffer.readUInt32LE(MEMORY_LAYOUT.CAPACITY_OFFSET);
      const totalEvents = this.buffer.readBigUInt64LE(MEMORY_LAYOUT.TOTAL_EVENTS_OFFSET);
      const droppedEvents = this.buffer.readBigUInt64LE(MEMORY_LAYOUT.DROPPED_EVENTS_OFFSET);
      const producerPid = this.buffer.readUInt32LE(MEMORY_LAYOUT.PRODUCER_PID_OFFSET);
      const maxUtilization = this.buffer.readUInt32LE(MEMORY_LAYOUT.MAX_UTILIZATION_OFFSET);
      const avgLatency = this.buffer.readBigUInt64LE(MEMORY_LAYOUT.AVG_LATENCY_OFFSET);
